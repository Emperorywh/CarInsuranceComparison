...
| 14 | AI 范围 | **MVP 纯规则引擎** | 差异分析、五问总结、价格归因全部规则实现；LLM 仅用于解析 |
...
> COMPULSORY 仅作识别映射码：交强险保费与车船税只落 quote 价格字段（`compulsoryPremium` / `vehicleTax`），不生成 quote_coverage 行；其状态、置信度和来源分别保存在价格状态字段与 field_evidence，五问与对比直接读取这些字段。
...
- 两个方案时按用户勾选顺序，以第一个为差异基准；三个及以上方案仍以第一个为固定基准，页面明确显示「对比基准」，避免变化箭头含义不清。第四问归因固定以最低净支出方案为基准：两者不是同一基准时页面分别标注基准身份，互不改写。

### 7.2 五问总结（对比页第一屏，PRD 65 节）

1. **哪个最便宜**：按 netPayment 排序，标注最低者；若最低值包含用户估值或总额校验异常，文案改为「按当前已确认金额暂为最低」。全部报价均无总价时回答「价格信息不足」。
2. **哪些关键保障额度最高**：三者险、车损保额、三者医保外、司机医保外、乘客医保外分别比较并逐项标注，不把不同保障对象直接求和。车损保额只描述报价差异，不表述为“保额越高方案越好”。
3. **哪些方案保障不完整**：商业四大主险（车损/三者/司机/乘客）任一缺失或 status ∈ {NOT_INCLUDED, UNKNOWN} → 标注「保障不完整」并给出缺失清单。**交强险不计入**完整性判定（纯商业险报价合法，见 §12）。
4. **贵的钱贵在哪里**：以最低净支出方案为基准，逐项拆解 Δ商业险、Δ交强、Δ车船税、Δ保障包；只有双方明细保费完整时才给出险种 Top 变化，否则明确显示「明细保费不完整，无法继续拆分」。
5. **哪些不能直接比**：核心险种集合、状态、保额、单座金额、座位数、共享保额、倍数或生效条件任一不同 → 「同口径提示：核心保障口径不同，不能仅按总价判断」（PRD 41 节）。UNKNOWN 直接标注「信息不足，暂无法比较」。任一方案存在已确认保留的未识别金额项时，提示「N 项未识别保障未参与结构化对比」；未识别项不进入 7.4 分区，仅在此提示。

### 7.3 差异标签

`↑ 增加 / ↓ 减少 / + 新增 / − 缺失 / = 相同`，逐行渲染（数值行带变化前后值，如「200万 → 300万 ↑」）。

### 7.4 对比分区

价格 → 核心保障 → 附加险 → 额外保障（保障包展开内部 coverage）→ 增值服务 → 优惠/净支出。差异行默认高亮置顶；「相同」行折叠。
...
| `/projects/[id]/compare` | **对比页**（核心） | 选择报价（上限 6）→ 第一屏五问总结卡片（§7.2）→ 分区对比：**方案列横向滑动**（冻结首列指标名，方案卡片列宽 ~44vw），行=指标；明确显示差异基准；差异行高亮置顶、相同行折叠；总价异常、信息不足和用户估值不得隐藏；右上角「导出长图」 |
| 补传合并预览（confirm 页内） | MERGE_REVIEW | 变更清单逐条「采纳新值 / 保留旧值」，全部处理完才回到 CONFIRMED（决策：增量差异确认） |
| 导出长图 | — | 对比总结渲染为竖版长图（html→canvas，如 html-to-image）：五问 + 价格表 + 核心差异，**脱敏**（不含任何个人信息），保存/分享 |
...
| GET | `/projects/{id}/compare?quoteIds=a,b,c` | 对比结果（五问 + 分区 + 差异标签） |
...
web/
  ...
  components/compare/  # 五问卡片、横滑对比表、差异标签
...
**做**（= PRD 60 节清单 + 访谈新增）：
项目 CRUD；报价容器创建（选公司+保险员）；图片/PDF 多文件上传；异步解析+进度及重启恢复；多方案拆分；分层确认页（7 Tab、编辑增删、文件+页码+原文摘录定位、三档置信度）；纯手动报价；补传增量合并；优惠/返现与净支出；五问总结+六区对比+差异标签+同口径提示；导出长图；总额三态校验；解析告知、脱敏和本地访问控制。
