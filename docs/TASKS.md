...
- LLM 只负责报价结构化抽取；归一化、校验、置信度、价格计算、对比总表与差异标签全部由确定性规则完成。
...
- **状态**：`[x] 已完成`
- **目标**：按产品决策（2026-09）把对比页从「五问总结 + 六分区卡片」改为单一总表，后端契约、对比页与导出长图三者同构；破坏性契约变更，不保留旧字段兼容。
- **前置依赖**：TASK-06/07 已交付。
- **实施范围**：
  1. 后端契约删除 `fiveQuestions` 与 `sections`，`ComparisonResult` 改为直接下发平铺 `rows`（价格 → 核心保障 → 附加险 → 额外保障 → 增值服务 → 优惠/净支出，组内差异行置顶）；五问相关 schema（11 个）与引擎计算逻辑全部移除，快照中仅为归因服务的 `computedCommercialPremium` 投影一并删除（ORM 字段保留）。
  2. 原第五问「N 项未识别保障未参与结构化对比」提示改为方案列异常标注，数据完整性警告不丢失。
  3. 前端对比页删除五问卡片与 `five-questions.tsx`，`CompareTables` 直接渲染 `rows` 为一张表（一个表头、相同行全局折叠）；基准说明改「价格基准（净支出最低）」。
  4. 导出长图删除「五问总结」「核心差异」板块，改为与页面同构的单一总表（方案表头 + 全部指标行，差异行高亮），隐私白名单边界不变（agentName 等仍结构性排除）。
  5. 文档同步：`docs/SPEC_MVP.md`（§0 决策总览、§3、§7 对比引擎、§8 页面、§10 API、§11 目录、§14 MVP 范围）与 `docs/PRD.md`（§33–43、§64–65）改为单一总表口径。
- **主要交付物**：新对比契约（`api/openapi.json` + `web/lib/api-types.d.ts` 随 `pnpm gen:api` 再生）、单一总表页面与长图画布、重构后的引擎纯函数（分区构建改平铺行构建）与测试。
- **明确不做**：不保留旧字段兼容层；不新增 AI 分析能力（分析型问题仍属第二阶段，PRD §66）。
- **验证**：
  1. 后端：`uv run ruff check .` 通过；`uv run pytest tests/test_comparison_engine.py tests/test_compare_api.py` 全过（12 + 11 例，含 6 报价 × 200 明细对比 P95 < 500ms 口径；引擎测试改为分组顺序/组内置顶/序列化契约断言）。
  2. 前端：`tsc --noEmit` 与 ESLint 通过；vitest 全量 **90/90 连续 5 轮通过**（夹具改为 `rows` 契约；顺带修复 export-button 集成测试中「栅格化完成后按钮复位」的既有异步竞态，改为 waitFor）。
  3. `pnpm gen:api` + `pnpm check:api` 契约零漂移；Playwright 主路径「3 报价对比 + 导出长图」通过（真实下载合法非空 PNG）。
  4. 浏览器实测 `/projects/2/compare?quoteIds=4,5`：单一总表渲染、接口仅含 `rows` 无 `fiveQuestions`/`sections`、方案列出现「4 项未识别保障未参与对比」标注。
- **完成判定**：2–6 报价对比在接口、页面与导出长图中呈现同一张总表；契约与文档中不存在五问/分区口径。
...
| 规则对比引擎、单一总表、差异与移动端对比 | TASK-06、TASK-08 | 导出与第二阶段 AI 只消费 rows 契约 |
| 长图、E2E、真实样本、隐私/性能交付 | TASK-07 | MVP 到此结束，不进入第二阶段 |

这 8 个 Task 的边界分别落在数据契约、人工数据、文件/任务、模型候选、生命周期合并、只读比较、发布门禁和对比呈现改版上。合并任意相邻两项都会让单个上下文同时承担两个高风险状态边界；继续拆分则会产生无法独立验收的纯脚手架任务，因此不再细分。
